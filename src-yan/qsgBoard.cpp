...
static QHash<qsgModel*, QHash<QString, QImage>> m_image_caches;
static QHash<qsgModel*, QString> m_model_ids;

QHash<QString, QImage>* getImageCache(qsgModel* aModel){
    return rea::tryFind(&m_image_caches, aModel);
}

void updateImageCache(qsgModel* aModel){
    rea::pipeline::run(m_model_ids.value(aModel), getImageCache(aModel));
}
...
void qsgBoard::beforeDestroy(){
    if (m_models.size() > 0){
        ...
        while (m_models.size() > 1){
            m_image_caches.remove(m_models.front().get());
            m_model_ids.remove(m_models.front().get());
            m_models.pop_front();
        }
        m_updates.clear();
...
bool tryFlushImageCache(const QJsonObject& aModification, QHash<QString, QImage>* aCache = nullptr){
...
void qsgBoard::setName(const QString& aName){
    ...
    rea::pipeline::add<QJsonObject>([this](rea::stream<QJsonObject>* aInput){
        ...
        auto mdl = std::make_shared<qsgModel>(rea::Json(dt, "model_id", id));
        m_model_ids.insert(mdl.get(), id);
        auto che = getImageCache(mdl.get());
    ...
    rea::pipeline::add<QJsonObject, pipeDelegate>([this](rea::stream<QJsonObject>* aInput){
        ...
        if (m_models.size() > 0){
            if (tryFlushImageCache(aInput->data(), getImageCache(m_models.back().get())))
                updateImageCache(m_models.back().get());
    ...
    rea::pipeline::add<QJsonArray, pipeDelegate>([this](rea::stream<QJsonArray>* aInput){
        ...
        if (m_models.size() > 0){
            for (auto i : dt){
                auto mdy = i.toObject();
                if (tryFlushImageCache(mdy, getImageCache(m_models.back().get())))
                    updateImageCache(m_models.back().get());
...
QSGNode* qsgBoard::updatePaintNode(QSGNode* aOldNode, UpdatePaintNodeData* nodedata){
    ...
    if (m_models.size() > 1){
        ...
        while (m_models.size() > 1){
            m_image_caches.remove(m_models.front().get());
            m_model_ids.remove(m_models.front().get());
            //std::cout << "clear: " << m_models.size() << std::endl;
            m_models.pop_front();
        }
    }
    if (m_models.size() == 1){
